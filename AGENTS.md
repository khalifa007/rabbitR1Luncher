...
- **OkHttp 4.12.0**, **kotlinx-serialization-json 1.7.3**, **security-crypto 1.1.0-alpha06**, **bcprov-jdk18on 1.78.1**, **zxing-android-embedded 4.3.0** — for the OpenClaw chat panel. STT goes through OpenAI Whisper (cloud), not on-device Vosk.
...
**Panel state machine** (see CLAUDE.md for the canonical list — this file is a higher-level overview): `state.panel` cycles through HOME, APPS, SETTINGS + sub-panels (NETWORK, BRIGHTNESS, VOLUME, FACTORY_CONFIRM, WIFI_*), the OPENCLAW_* family, and MESSAGES. Each panel has its own focus int; brightness/volume use level fields instead. The `back()` unwind is intentional and asymmetric:

- `BRIGHTNESS|VOLUME → SETTINGS → APPS → HOME` (Settings exits back into the apps grid since it's an in-grid app)
- `OPENCLAW_QR|OPENCLAW_CHAT → APPS` (in-grid synthetic apps)
- `OPENCLAW_SETTINGS → OPENCLAW_CHAT` (settings is a child of chat, not of the grid)

**Apps list is typed**: `state.apps` is `MutableList<AppEntry>` — a sealed class: `Real(ResolveInfo)`, `Settings`, `OpenClaw`, `Messages`. `LauncherActivity.loadApps()` appends the synthetics in this order: real apps, then `Messages`, then `OpenClaw`, then `Settings`. `launchApp(idx)` switches on the type:
...
- `Messages` — `READ_SMS` grant check then `state.openMessages()` + `loadSmsConversations()`
...
**Navigation actions** live in `LauncherNav.kt` as extension functions on `LauncherState`: `wheelUp(host)`, `wheelDown(host)`, `activate(host)`, `backPressed(host)`. They're pure state mutations; side effects (starting intents, tones, brightness/volume writes, OpenClaw record) go through the `LauncherHost` interface which `LauncherActivity` implements. The host covers: app launch, system intents, brightness/volume, tones, OpenClaw (`openClawScanned`, `openClawToggleRecord`, `openClawSendText`, `openClawScrollUp/Down`, `openClawCloseSession`, `openClawPasteOpenaiKey`, `openClawClearOpenaiKey`, `openClawSaveOpenaiKey`, camera + motor nudge), SMS load/open, and the web-server toggle.

**Compose tree** (z-stack inside a single `Box` in `LauncherRoot.kt`):

```
wallpaper
  + HomeScreen          (always composed; clock + date)
  + AppsPanel           (panel == APPS)
  + SettingsPanel       (panel == SETTINGS) + sub-panels (NETWORK, BRIGHTNESS, VOLUME, FACTORY_CONFIRM, WIFI_*)
  + OpenClaw* panels    (QR, CHAT, TALK, CANVAS, CAMERA, SETTINGS, SESSIONS)
  + MessagesPanel + MessagesThreadPanel
  + Topbar              (HOME only)
```
...
| `AudioCapture.kt` | Pure mic → WAV (16 kHz mono PCM_16BIT, capped at 30 s). Used for voice input. |
...
- `drawable/` — vector/shape drawables loaded via `painterResource(R.drawable.ic_xxx)`. The synthetic apps reuse existing icons: Settings → `ic_settings`, OpenClaw → `ic_wifi_arc`, Messages → `ic_messages`.
